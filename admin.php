...
function normalizeDomainValueSettings($rawSettings)
{
    ...
    if ($settings['slogan'] === '') {
        $settings['slogan'] = $defaults['slogan'];
    }
    if (empty($settings['items'])) {
        $settings['items'] = $defaults['items'];
    }

    $settings['items'] = array_slice($settings['items'], 0, 6);
    return $settings;
}
...
function getFooterLinksFromSettings($data)
{
    ...
    return normalizeFooterLinks([
        [
            'name' => 'WHOIS查询',
            'url' => $data['footer_whois_url'] ?? 'https://who.ga/',
    ...
    switch ($_POST['action']) {
        case 'get_email_settings':
            ...
            echo json_encode(['success' => true, 'data' => [
                ...
                'email_provider' => in_array($data['email_provider'] ?? 'smtp', ['smtp', 'sendflare', 'resend'], true) ? ($data['email_provider'] ?? 'smtp') : 'smtp',
                'smtp_enabled' => true,
                ...
                'sendflare_api_key' => $data['sendflare_api_key'] ?? '',
        ...
        case 'save_email_settings':
            ...
            if (!in_array($emailProvider, ['smtp', 'sendflare', 'resend'], true)) {
                $emailProvider = 'smtp';
            }
            $smtpEnabled = true;
            ...
            $sendflareApiKey = trim($_POST['sendflare_api_key'] ?? '');
            $resendApiKey = trim($_POST['resend_api_key'] ?? '');
            ...
            if ($emailProvider === 'smtp' && $smtpHost === '') {
                echo json_encode(['success' => false, 'message' => '请选择 SMTP 时需要填写 SMTP 服务器']);
                exit;
            }
            if ($emailProvider === 'sendflare' && $sendflareApiKey === '') {
                echo json_encode(['success' => false, 'message' => '请选择 Sendflare 时需要填写 API Key']);
                exit;
            }
            if ($emailProvider === 'resend' && $resendApiKey === '') {
                echo json_encode(['success' => false, 'message' => '请选择 Resend 时需要填写 API Key']);
                exit;
            }
            $settingsFile = __DIR__ . '/data/email_settings.json';
            writeJsonFile($settingsFile, [
                ...
                'sendflare_api_key' => $sendflareApiKey,
        ...
        case 'get_domain_value_settings':
            $settingsFile = __DIR__ . '/data/site_settings.json';
            $data = readJsonFile($settingsFile);
            echo json_encode(['success' => true, 'data' => normalizeDomainValueSettings($data['domain_value'] ?? [])], JSON_UNESCAPED_UNICODE);
            exit;

        case 'save_domain_value_settings':
            $settingsFile = __DIR__ . '/data/site_settings.json';
            $existingData = readJsonFile($settingsFile);
            $rawSettings = (string)($_POST['domain_value_json'] ?? '{}');
            $decoded = json_decode($rawSettings, true);

            if (!is_array($decoded)) {
                echo json_encode(['success' => false, 'message' => '价值卡片数据格式无效'], JSON_UNESCAPED_UNICODE);
                exit;
            }

            $items = $decoded['items'] ?? [];
            if (!is_array($items) || count($items) < 1 || count($items) > 6) {
                echo json_encode(['success' => false, 'message' => '价值卡片条目需为 1 到 6 个'], JSON_UNESCAPED_UNICODE);
                exit;
            }

            foreach ($items as $index => $item) {
                if (!is_array($item)) {
                    echo json_encode(['success' => false, 'message' => '第 ' . ($index + 1) . ' 个条目格式无效'], JSON_UNESCAPED_UNICODE);
                    exit;
                }
                if (trim((string)($item['title'] ?? '')) === '' || trim((string)($item['description'] ?? '')) === '') {
                    echo json_encode(['success' => false, 'message' => '第 ' . ($index + 1) . ' 个条目的标题和描述不能为空'], JSON_UNESCAPED_UNICODE);
                    exit;
                }
                if (!isValidFontAwesomeClass(trim((string)($item['icon_class'] ?? '')))) {
                    echo json_encode(['success' => false, 'message' => '第 ' . ($index + 1) . ' 个条目的图标类名无效'], JSON_UNESCAPED_UNICODE);
                    exit;
                }
            }

            $newData = $existingData;
            $newData['domain_value'] = normalizeDomainValueSettings($decoded);
            writeJsonFile($settingsFile, $newData);
            echo json_encode(['success' => true, 'message' => '价值卡片设置已保存'], JSON_UNESCAPED_UNICODE);
            exit;
        ...
        case 'get_domains_page':
            $ajaxPerPage = max(10, min(50, (int)($_POST['per_page'] ?? 10)));
            $ajaxPage = max(1, (int)($_POST['page'] ?? 1));
            $ajaxDomains = $domainManager->getAllDomains();
            $ajaxCount = count($ajaxDomains);
            $ajaxTotalPages = max(1, (int)ceil($ajaxCount / $ajaxPerPage));
            $ajaxPage = min($ajaxPage, $ajaxTotalPages);
            $ajaxOffset = ($ajaxPage - 1) * $ajaxPerPage;
            $ajaxRows = array_slice($ajaxDomains, $ajaxOffset, $ajaxPerPage);

            echo json_encode([
                'success' => true,
                'data' => [
                    'domains' => $ajaxRows,
                    'total_count' => $ajaxCount,
                    'current_page' => $ajaxPage,
                    'total_pages' => $ajaxTotalPages,
                    'per_page' => $ajaxPerPage,
                ],
            ], JSON_UNESCAPED_UNICODE);
            exit;
...
$allowedSections = ['domains', 'stats', 'email', 'site', 'value', 'footer'];
...
<body data-admin-endpoint="<?php echo htmlspecialchars($adminAjaxEndpoint, ENT_QUOTES, 'UTF-8'); ?>" data-csrf-token="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <div class="admin-scroll">
        <div class="admin-layout admin-container">
            ...
            <aside class="sidebar">
                <nav class="menu">
                    <a class="menu-item <?php echo $section === 'domains' ? 'active' : ''; ?>" href="<?php echo htmlspecialchars(adminSectionUrl('domains'), ENT_QUOTES, 'UTF-8'); ?>"><i class="menu-icon fa-solid fa-globe" aria-hidden="true"></i><span>域名管理</span></a>
                    <a class="menu-item <?php echo $section === 'stats' ? 'active' : ''; ?>" href="<?php echo htmlspecialchars(adminSectionUrl('stats'), ENT_QUOTES, 'UTF-8'); ?>"><i class="menu-icon fa-solid fa-chart-line" aria-hidden="true"></i><span>访问统计</span></a>
                    <a class="menu-item <?php echo $section === 'email' ? 'active' : ''; ?>" href="<?php echo htmlspecialchars(adminSectionUrl('email'), ENT_QUOTES, 'UTF-8'); ?>"><i class="menu-icon fa-solid fa-envelope" aria-hidden="true"></i><span>邮件设置</span></a>
                    <a class="menu-item <?php echo $section === 'site' ? 'active' : ''; ?>" href="<?php echo htmlspecialchars(adminSectionUrl('site'), ENT_QUOTES, 'UTF-8'); ?>"><i class="menu-icon fa-solid fa-gear" aria-hidden="true"></i><span>站点设置</span></a>
                    <a class="menu-item <?php echo $section === 'value' ? 'active' : ''; ?>" href="<?php echo htmlspecialchars(adminSectionUrl('value'), ENT_QUOTES, 'UTF-8'); ?>"><i class="menu-icon fa-solid fa-layer-group" aria-hidden="true"></i><span>价值卡片</span></a>
                    <a class="menu-item <?php echo $section === 'footer' ? 'active' : ''; ?>" href="<?php echo htmlspecialchars(adminSectionUrl('footer'), ENT_QUOTES, 'UTF-8'); ?>"><i class="menu-icon fa-solid fa-link" aria-hidden="true"></i><span>页脚设置</span></a>
                </nav>
            </aside>
                        ...
                        <div class="pagination-wrapper">
                            <div class="pagination">
                                <?php if ($currentPage > 1): ?>
                                    <a href="<?php echo htmlspecialchars(adminSectionUrl('domains', ['page' => $currentPage - 1, 'per_page' => $perPage]), ENT_QUOTES, 'UTF-8'); ?>" class="pagination-btn" data-domains-page="<?php echo $currentPage - 1; ?>">上一页</a>
                                <?php else: ?>
                                ...
                                <?php if ($currentPage < $totalPages): ?>
                                    <a href="<?php echo htmlspecialchars(adminSectionUrl('domains', ['page' => $currentPage + 1, 'per_page' => $perPage]), ENT_QUOTES, 'UTF-8'); ?>" class="pagination-btn" data-domains-page="<?php echo $currentPage + 1; ?>">下一页</a>
                                <?php else: ?>
            ...
            <?php elseif ($section === 'email'): ?>
                <div class="card email-settings-card">
                    <form id="emailSettingsForm" class="email-settings-form" onsubmit="saveEmailSettings(event)">
                        ...
                        <section class="email-settings-section">
                            ...
                            <div class="form-group">
                                <label for="email_provider">发送服务商</label>
                                <select id="email_provider" name="email_provider">
                                    <option value="smtp">SMTP</option>
                                    <option value="sendflare">Sendflare API</option>
                                    <option value="resend">Resend API</option>
                                </select>
                            </div>

                            <div class="email-provider-fields" data-provider-fields="smtp">
                            ...
                            <div class="email-provider-fields" data-provider-fields="sendflare">
                                <div class="form-group">
                                    <label for="sendflare_api_key">Sendflare API Key</label>
                                    <input type="password" id="sendflare_api_key" name="sendflare_api_key" placeholder="live_...">
                                    <small style="color: var(--text-secondary); font-size: 13px; margin-top: 4px; display: block;">使用 Sendflare Transactional API：POST https://api.sendflare.com/v1/send。</small>
                                </div>
                            </div>

                            <div class="email-provider-fields" data-provider-fields="resend">
                                <div class="form-group">
                                    <label for="resend_api_key">Resend API Key</label>
                                    <input type="password" id="resend_api_key" name="resend_api_key" placeholder="re_...">
                                    <small style="color: var(--text-secondary); font-size: 13px; margin-top: 4px; display: block;">使用 Resend Email API：POST https://api.resend.com/emails。</small>
                                </div>
                            </div>
                        </section>
                ...
                <div class="card">
                    <h2>说明</h2>
                    <p>系统支持 SMTP、Sendflare API 和 Resend API。默认收件人邮箱用于接收客户的购买咨询邮件。使用 API 服务时，请先在对应平台完成发信域名验证。</p>
                </div>
            ...
            <?php elseif ($section === 'value'): ?>
                <div class="card">
                    <form id="domainValueSettingsForm" onsubmit="saveDomainValueSettings(event)">
                        <div class="form-group">
                            <label for="domain_value_title">标题</label>
                            <input type="text" id="domain_value_title" name="domain_value_title" placeholder="一个好域名，一份好事业">
                        </div>
                        <div class="form-group">
                            <label for="domain_value_text">说明文字</label>
                            <textarea id="domain_value_text" name="domain_value_text" rows="3" placeholder="域名是互联网世界的唯一标识..."></textarea>
                        </div>
                        <div class="form-group">
                            <label>条目</label>
                            <small style="color: var(--text-secondary); font-size: 13px; display: block; margin-bottom: 10px;">
                                图标填写 Font Awesome 类名，例如 <code>fa-solid fa-chart-line</code>、<code>fa-solid fa-building</code>。最多 6 个条目。
                            </small>
                            <div id="domainValueItemsContainer" class="footer-links-editor"></div>
                            <button type="button" class="btn btn-secondary" onclick="addDomainValueItemRow()" style="margin-top: 12px;">添加条目</button>
                        </div>
                        <div class="form-group">
                            <label for="domain_value_slogan">底部标语</label>
                            <textarea id="domain_value_slogan" name="domain_value_slogan" rows="2" placeholder="把握机会，拥有这个优质域名..."></textarea>
                        </div>
                        <div class="modal-footer" style="justify-content:flex-end; gap:12px;">
                            <button type="submit" class="btn btn-primary">保存设置</button>
                        </div>
                ...
                <div class="card">
                    <h2>说明</h2>
                    <p>这里控制首页“一个好域名，一份好事业”卡片。图标只保存 Font Awesome 类名，不支持粘贴 SVG 或脚本代码。</p>
                </div>
    ...
    <footer class="admin-footer">
        <div class="admin-footer-content">
            <p class="admin-footer-copyright">
                &copy; <?php echo date('Y'); ?> <a href="https://namedeal.io" target="_blank" rel="noopener noreferrer">域名管理系统</a>
            </p>
