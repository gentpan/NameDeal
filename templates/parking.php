...
<body>
    ...
    <div class="container">
        ...
        $domainValue = (isset($domainValueSettings) && is_array($domainValueSettings) && !empty($domainValueSettings))
            ? array_replace($defaultDomainValueSettings, $domainValueSettings)
            : $defaultDomainValueSettings;
        $domainValueItems = isset($domainValue['items']) && is_array($domainValue['items']) && !empty($domainValue['items'])
            ? array_slice($domainValue['items'], 0, 6)
            : $defaultDomainValueSettings['items'];
        $isValueFaIcon = static function ($iconClass) {
            return preg_match('/^(fa-(solid|regular|brands|duotone|thin|light)\s+)?fa-[a-z0-9-]+(?:\s+fa-[a-z0-9-]+)*$/i', trim((string)$iconClass));
        };
        ?>
        <div class="domain-value">
            <div class="value-content">
                <h3 class="value-title"><?php echo htmlspecialchars((string)$domainValue['title'], ENT_QUOTES, 'UTF-8'); ?></h3>
                <p class="value-text"><?php echo htmlspecialchars((string)$domainValue['text'], ENT_QUOTES, 'UTF-8'); ?></p>
                <ul class="value-list">
                    <?php foreach ($domainValueItems as $item): ?>
                        <?php
                        $valueIconClass = trim((string)($item['icon_class'] ?? 'fa-solid fa-circle-check'));
                        if (!$isValueFaIcon($valueIconClass)) {
                            $valueIconClass = 'fa-solid fa-circle-check';
                        }
                        ?>
                        <li>
                            <div class="value-item-icon">
                                <i class="list-icon <?php echo htmlspecialchars($valueIconClass, ENT_QUOTES, 'UTF-8'); ?>" aria-hidden="true"></i>
                            </div>
                            <div class="value-item-content">
                                <strong><?php echo htmlspecialchars((string)($item['title'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></strong>
                                <span class="value-item-desc"><?php echo htmlspecialchars((string)($item['description'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></span>
                            </div>
                        </li>
                    <?php endforeach; ?>
                    <?php if (empty($domainValueItems)): ?>
                        <li>
                        <div class="value-item-icon">
                            <i class="list-icon fa-solid fa-circle-check" aria-hidden="true"></i>
                        </div>
                        <div class="value-item-content">
                ...
                <p class="value-slogan"><?php echo htmlspecialchars((string)$domainValue['slogan'], ENT_QUOTES, 'UTF-8'); ?></p>
            </div>
        </div>
