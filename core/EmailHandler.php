...
/**
 * 邮件 API 客户端
 * 支持 Sendflare 与 Resend 的 HTTP JSON 发送方式
 */
class ApiEmailClient
{
    private $timeout = 15;

    private function postJson($url, $apiKey, $payload)
    {
        if (!function_exists('curl_init')) {
            return ['success' => false, 'message' => '当前 PHP 环境未启用 curl 扩展'];
        }
        ...
        return [
            ...
            'raw' => is_string($responseBody) ? $responseBody : '',
            'error' => $error,
        ];
    }
    ...
    private function extractErrorMessage($response, $fallback)
    {
        ...
        if (!empty($response['data']['error'])) {
            return is_string($response['data']['error']) ? $response['data']['error'] : json_encode($response['data']['error'], JSON_UNESCAPED_UNICODE);
        }
    ...
    public function sendWithSendflare($apiKey, $options)
    {
        $apiKey = trim((string)$apiKey);
        if ($apiKey === '') {
            return ['success' => false, 'message' => 'Sendflare API Key 未配置'];
        }
        ...
        $response = $this->postJson('https://api.sendflare.com/v1/send', $apiKey, $payload);
        $success = $response['success'] && (!array_key_exists('success', $response['data']) || !empty($response['data']['success']));
        if ($success) {
            return ['success' => true, 'message' => '邮件发送成功（Sendflare）'];
        }

        $message = $this->extractErrorMessage($response, 'Sendflare 邮件发送失败');
        error_log('Sendflare send failed: HTTP ' . $response['http_code'] . ' ' . $message);
        return ['success' => false, 'message' => $message];
    }

    public function sendWithResend($apiKey, $options)
    {
        $apiKey = trim((string)$apiKey);
        if ($apiKey === '') {
        ...
        $response = $this->postJson('https://api.resend.com/emails', $apiKey, $payload);
        if ($response['success'] && !empty($response['data']['id'])) {
            return ['success' => true, 'message' => '邮件发送成功（Resend）'];
        }

        $message = $this->extractErrorMessage($response, 'Resend 邮件发送失败');
        error_log('Resend send failed: HTTP ' . $response['http_code'] . ' ' . $message);
        return ['success' => false, 'message' => $message];
    }
}
...
class EmailHandler
{
    ...
    /**
     * 发送邮件
     * 根据后台选择的服务商发送邮件，支持 SMTP、Sendflare 和 Resend
    ...
    public function sendEmail($subject, $message, $fromEmail = null, $fromName = null, $replyToEmail = null, $toEmail = null)
    {
        ...
        if (!in_array($provider, ['smtp', 'sendflare', 'resend'], true)) {
            $provider = 'smtp';
        }
        ...
        if ($provider === 'sendflare') {
            $client = new ApiEmailClient();
            $result = $client->sendWithSendflare($this->settings['sendflare_api_key'] ?? '', $apiOptions);
            return $result;
        }

        if ($provider === 'resend') {
            $client = new ApiEmailClient();
            $result = $client->sendWithResend($this->settings['resend_api_key'] ?? '', $apiOptions);
            return $result;
        }
        ...
        if ($provider === 'smtp') {
            ...
            if ($smtpHost) {
                ...
                error_log('SMTP发送失败');
                return ['success' => false, 'message' => 'SMTP 邮件发送失败，请检查服务器、账号、密码和端口配置'];
            }

            return ['success' => false, 'message' => 'SMTP 服务器未配置'];
        }

        return ['success' => false, 'message' => '邮件发送服务商配置无效'];
    }
